// Code generated by entc, DO NOT EDIT.

package filemiddleware

import (
	"digidrop/ent/predicate"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"github.com/google/uuid"
)

// ID filters vertices based on their ID field.
func ID(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldID), id))
	})
}

// IDEQ applies the EQ predicate on the ID field.
func IDEQ(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldID), id))
	})
}

// IDNEQ applies the NEQ predicate on the ID field.
func IDNEQ(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldID), id))
	})
}

// IDIn applies the In predicate on the ID field.
func IDIn(ids ...uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(ids) == 0 {
			s.Where(sql.False())
			return
		}
		v := make([]interface{}, len(ids))
		for i := range v {
			v[i] = ids[i]
		}
		s.Where(sql.In(s.C(FieldID), v...))
	})
}

// IDNotIn applies the NotIn predicate on the ID field.
func IDNotIn(ids ...uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(ids) == 0 {
			s.Where(sql.False())
			return
		}
		v := make([]interface{}, len(ids))
		for i := range v {
			v[i] = ids[i]
		}
		s.Where(sql.NotIn(s.C(FieldID), v...))
	})
}

// IDGT applies the GT predicate on the ID field.
func IDGT(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldID), id))
	})
}

// IDGTE applies the GTE predicate on the ID field.
func IDGTE(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldID), id))
	})
}

// IDLT applies the LT predicate on the ID field.
func IDLT(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldID), id))
	})
}

// IDLTE applies the LTE predicate on the ID field.
func IDLTE(id uuid.UUID) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldID), id))
	})
}

// URLID applies equality check predicate on the "url_id" field. It's identical to URLIDEQ.
func URLID(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldURLID), v))
	})
}

// FilePath applies equality check predicate on the "file_path" field. It's identical to FilePathEQ.
func FilePath(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldFilePath), v))
	})
}

// Accessed applies equality check predicate on the "accessed" field. It's identical to AccessedEQ.
func Accessed(v bool) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldAccessed), v))
	})
}

// URLIDEQ applies the EQ predicate on the "url_id" field.
func URLIDEQ(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldURLID), v))
	})
}

// URLIDNEQ applies the NEQ predicate on the "url_id" field.
func URLIDNEQ(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldURLID), v))
	})
}

// URLIDIn applies the In predicate on the "url_id" field.
func URLIDIn(vs ...string) predicate.FileMiddleware {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.In(s.C(FieldURLID), v...))
	})
}

// URLIDNotIn applies the NotIn predicate on the "url_id" field.
func URLIDNotIn(vs ...string) predicate.FileMiddleware {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.NotIn(s.C(FieldURLID), v...))
	})
}

// URLIDGT applies the GT predicate on the "url_id" field.
func URLIDGT(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldURLID), v))
	})
}

// URLIDGTE applies the GTE predicate on the "url_id" field.
func URLIDGTE(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldURLID), v))
	})
}

// URLIDLT applies the LT predicate on the "url_id" field.
func URLIDLT(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldURLID), v))
	})
}

// URLIDLTE applies the LTE predicate on the "url_id" field.
func URLIDLTE(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldURLID), v))
	})
}

// URLIDContains applies the Contains predicate on the "url_id" field.
func URLIDContains(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.Contains(s.C(FieldURLID), v))
	})
}

// URLIDHasPrefix applies the HasPrefix predicate on the "url_id" field.
func URLIDHasPrefix(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.HasPrefix(s.C(FieldURLID), v))
	})
}

// URLIDHasSuffix applies the HasSuffix predicate on the "url_id" field.
func URLIDHasSuffix(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.HasSuffix(s.C(FieldURLID), v))
	})
}

// URLIDEqualFold applies the EqualFold predicate on the "url_id" field.
func URLIDEqualFold(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EqualFold(s.C(FieldURLID), v))
	})
}

// URLIDContainsFold applies the ContainsFold predicate on the "url_id" field.
func URLIDContainsFold(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.ContainsFold(s.C(FieldURLID), v))
	})
}

// FilePathEQ applies the EQ predicate on the "file_path" field.
func FilePathEQ(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldFilePath), v))
	})
}

// FilePathNEQ applies the NEQ predicate on the "file_path" field.
func FilePathNEQ(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldFilePath), v))
	})
}

// FilePathIn applies the In predicate on the "file_path" field.
func FilePathIn(vs ...string) predicate.FileMiddleware {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.In(s.C(FieldFilePath), v...))
	})
}

// FilePathNotIn applies the NotIn predicate on the "file_path" field.
func FilePathNotIn(vs ...string) predicate.FileMiddleware {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
	return predicate.FileMiddleware(func(s *sql.Selector) {
		// if not arguments were provided, append the FALSE constants,
		// since we can't apply "IN ()". This will make this predicate falsy.
		if len(v) == 0 {
			s.Where(sql.False())
			return
		}
		s.Where(sql.NotIn(s.C(FieldFilePath), v...))
	})
}

// FilePathGT applies the GT predicate on the "file_path" field.
func FilePathGT(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GT(s.C(FieldFilePath), v))
	})
}

// FilePathGTE applies the GTE predicate on the "file_path" field.
func FilePathGTE(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.GTE(s.C(FieldFilePath), v))
	})
}

// FilePathLT applies the LT predicate on the "file_path" field.
func FilePathLT(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LT(s.C(FieldFilePath), v))
	})
}

// FilePathLTE applies the LTE predicate on the "file_path" field.
func FilePathLTE(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.LTE(s.C(FieldFilePath), v))
	})
}

// FilePathContains applies the Contains predicate on the "file_path" field.
func FilePathContains(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.Contains(s.C(FieldFilePath), v))
	})
}

// FilePathHasPrefix applies the HasPrefix predicate on the "file_path" field.
func FilePathHasPrefix(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.HasPrefix(s.C(FieldFilePath), v))
	})
}

// FilePathHasSuffix applies the HasSuffix predicate on the "file_path" field.
func FilePathHasSuffix(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.HasSuffix(s.C(FieldFilePath), v))
	})
}

// FilePathEqualFold applies the EqualFold predicate on the "file_path" field.
func FilePathEqualFold(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EqualFold(s.C(FieldFilePath), v))
	})
}

// FilePathContainsFold applies the ContainsFold predicate on the "file_path" field.
func FilePathContainsFold(v string) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.ContainsFold(s.C(FieldFilePath), v))
	})
}

// AccessedEQ applies the EQ predicate on the "accessed" field.
func AccessedEQ(v bool) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldAccessed), v))
	})
}

// AccessedNEQ applies the NEQ predicate on the "accessed" field.
func AccessedNEQ(v bool) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldAccessed), v))
	})
}

// HasFileMiddlewareToUser applies the HasEdge predicate on the "FileMiddlewareToUser" edge.
func HasFileMiddlewareToUser() predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FileMiddlewareToUserTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, FileMiddlewareToUserTable, FileMiddlewareToUserColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasFileMiddlewareToUserWith applies the HasEdge predicate on the "FileMiddlewareToUser" edge with a given conditions (other predicates).
func HasFileMiddlewareToUserWith(preds ...predicate.User) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FileMiddlewareToUserInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, FileMiddlewareToUserTable, FileMiddlewareToUserColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
		})
	})
}

// And groups predicates with the AND operator between them.
func And(predicates ...predicate.FileMiddleware) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s1 := s.Clone().SetP(nil)
		for _, p := range predicates {
			p(s1)
		}
		s.Where(s1.P())
	})
}

// Or groups predicates with the OR operator between them.
func Or(predicates ...predicate.FileMiddleware) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		s1 := s.Clone().SetP(nil)
		for i, p := range predicates {
			if i > 0 {
				s1.Or()
			}
			p(s1)
		}
		s.Where(s1.P())
	})
}

// Not applies the not operator on the given predicate.
func Not(p predicate.FileMiddleware) predicate.FileMiddleware {
	return predicate.FileMiddleware(func(s *sql.Selector) {
		p(s.Not())
	})
}
