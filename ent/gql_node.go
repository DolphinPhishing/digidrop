...
func (fm *FileMiddleware) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     fm.ID,
		Type:   "FileMiddleware",
		Fields: make([]*Field, 3),
	...
	var buf []byte
	if buf, err = json.Marshal(fm.URLID); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(fm.FilePath); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(fm.Accessed); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "bool",
	...
	node.Edges[0] = &Edge{
		Type: "User",
		Name: "FileMiddlewareToUser",
	}
	err = fm.QueryFileMiddlewareToUser().
		Select(user.FieldID).
...
func (u *User) Node(ctx context.Context) (node *Node, err error) {
	...
	if buf, err = json.Marshal(u.Type); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "user.Type",
	...
	node.Edges[0] = &Edge{
		Type: "FileMiddleware",
		Name: "UserFromFileMiddleware",
	}
	err = u.QueryUserFromFileMiddleware().
		Select(filemiddleware.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	switch table {
	case filemiddleware.Table:
		n, err := c.FileMiddleware.Query().
			Where(filemiddleware.ID(id)).
			CollectFields(ctx, "FileMiddleware").
			Only(ctx)
		if err != nil {
			return nil, err
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	switch table {
	case filemiddleware.Table:
		nodes, err := c.FileMiddleware.Query().
			Where(filemiddleware.IDIn(ids...)).
			CollectFields(ctx, "FileMiddleware").
			All(ctx)
		if err != nil {
			return nil, err
