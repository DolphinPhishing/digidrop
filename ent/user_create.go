...
func (uc *UserCreate) AddUserFromFileMiddlewareIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddUserFromFileMiddlewareIDs(ids...)
	return uc
}

// AddUserFromFileMiddleware adds the "UserFromFileMiddleware" edges to the FileMiddleware entity.
func (uc *UserCreate) AddUserFromFileMiddleware(f ...*FileMiddleware) *UserCreate {
	ids := make([]uuid.UUID, len(f))
	for i := range f {
		ids[i] = f[i].ID
	}
	return uc.AddUserFromFileMiddlewareIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.UserFromFileMiddlewareIDs(); len(nodes) > 0 {
