...
type UserQuery struct {
	...
	// eager-loading edges.
	withUserFromFileMiddleware *FileMiddlewareQuery
...
// QueryUserFromFileMiddleware chains the current query on the "UserFromFileMiddleware" edge.
func (uq *UserQuery) QueryUserFromFileMiddleware() *FileMiddlewareQuery {
	query := &FileMiddlewareQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserFromFileMiddleware: uq.withUserFromFileMiddleware.Clone(),
...
// WithUserFromFileMiddleware tells the query-builder to eager-load the nodes that are connected to
// the "UserFromFileMiddleware" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserFromFileMiddleware(opts ...func(*FileMiddlewareQuery)) *UserQuery {
	query := &FileMiddlewareQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserFromFileMiddleware = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [1]bool{
			uq.withUserFromFileMiddleware != nil,
		}
	)
	...
	if query := uq.withUserFromFileMiddleware; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserFromFileMiddleware = []*FileMiddleware{}
		}
		query.withFKs = true
		query.Where(predicate.FileMiddleware(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserFromFileMiddlewareColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.file_middleware_file_middleware_to_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "file_middleware_file_middleware_to_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "file_middleware_file_middleware_to_user" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserFromFileMiddleware = append(node.Edges.UserFromFileMiddleware, n)
		}
	}
